...
class AIScribeSupabaseUser extends BaseAuthUser {
  AIScribeSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> aIScribeSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AIScribeSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
